...
public class PlayMovGestureFly: MonoBehaviour {
	...
	void Update(){
		...
		//hand is missing
		if (GesturePitchHandValuesFly.invalidhands == true) {
			chosen = 1;
		}
		
		//hand is returned
		if (GesturePitchHandValuesFly.invalidhands == false) {
			chosen = 2;
		}

		if (TriggerScore.mazeScore > 10 && TriggerScore.mazeScore < 14 ) {
			chosen = 3;

		}
	
		if (CollideRestart.reset == true) {
			chosen = 4;
		}
