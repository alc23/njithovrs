...
public class PickFruitHandData : MonoBehaviour {
	...
	public static string fileName = string.Format("PickFruitHandData" + DateTime.Today.ToString("MMddyyyy") + ".txt");
	...
	void Update (){
		...
		foreach (Hand hand in frame.Hands) {
			...
			if (hand.IsValid) {
				...
				sw.Write ("," + hand.PinchStrength);
				//score count
				sw.Write ("," + PickFruitScore.pickingScore);
				//hand x scale
				sw.Write ("," + PickFruitHandScaling.xmin);
				sw.Write ("," + PickFruitHandScaling.xmax);
				sw.Write ("," + PickFruitHandScaling.scaled);
