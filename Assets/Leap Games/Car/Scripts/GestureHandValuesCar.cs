...
public class GestureHandValuesCar : MonoBehaviour {
	...
	static float timer;
	public static string fileName = string.Format("GestureHandCarData" + DateTime.Today.ToString("MMddyyyy") + ".txt");

	StreamWriter sw = new StreamWriter ("Data/Car/" + fileName, true);
	...
	void Update (){
		...
		foreach (Hand hand in frame.Hands) {
			...
			//	Debug.Log (hand.SphereRadius);
				...
				//sw.Write ("," + PlayerPrefs.GetFloat ("sphereRadiusclose1"));
				//sw.Write ("," + PlayerPrefs.GetFloat ("sphereRadiusopen1"));

				sw.Write ("," + hand.PalmNormal.Roll);

				sw.Write ("," + CoinRotate.coinsCollected);
				sw.Write ("," + CoinRotate.lifetimeCoins);
				sw.Write ("," + Application.loadedLevelName);
