...
public class HorizPlayMov: MonoBehaviour {
	...
	void Update(){
		...
		//hand is missing
		if (HorizHandValuesFly.invalidhands == true) {
			chosen = 1;
		}
		
		//hand is returned
		if (HorizHandValuesFly.invalidhands == false) {
			chosen = 2;
		}

		if (TriggerScore.mazeScore > 3 && TriggerScore.mazeScore < 6) {
			chosen = 3;
		}
	
		if (TriggerScore.mazeScore > 10 && TriggerScore.mazeScore < 13) {
			chosen = 4;
		}
